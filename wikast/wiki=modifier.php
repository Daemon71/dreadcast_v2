...
if(empty($_SESSION['id'])) $_SESSION['statut'] = "visiteur";
	...
	//RECEPTION DU BON MESSAGE
	
	$sql = 'SELECT titre,contenu,categorie,mots FROM wikast_wiki_articles_tbl WHERE id="'.$_GET['id'].'"' ;
	$req = mysql_query($sql);
	$res = mysql_num_rows($req);
	...
	if($res != 0)
		...
		$erreurcategorie = mysql_result($req,0,categorie);
		$erreurmots = mysql_result($req,0,mots);
		list($erreurintro,$erreurarticle) = explode("[LIMITE-INTRODUCTION]",str_replace("<br />","",mysql_result($req,0,contenu)));
		}
	...
	if($_POST['article']!="") $erreurarticle = stripslashes(stripslashes(stripslashes($_POST['article'])));
	if($_POST['commentaire']!="") $erreurjustification = stripslashes(stripslashes(stripslashes($_POST['commentaire'])));
	if($_POST['mots']!="") $erreurmots = stripslashes(stripslashes(stripslashes($_POST['mots'])));
	
	if($erreurarticle == "") $erreurarticle = "Veuillez &eacute;crire entre les balises [paragraphe][/paragraphe]";
...
include('include/inc_head.php'); ?>
		...
		<div id="page2">
			...
			<form id="mainpage-forum" action="wiki=valider.php" method="post" name="poster">
				<div id="haut">
					<p class="titre"><?php print($erreurtitre); ?></p>
				</div>
				...
				<div id="contenu">
					...
					<div class="wiki_p"><em>Vous vous appr&ecirc;tez &agrave; modifier un article sur le WiKast. Celle-ci devra &ecirc;tre valid&eacute;e par les administrateurs avant de para&icirc;tre en ligne.<br /><br />
					...
					<div id="DCcode">
						...
						<a href="javascript:AddText2(\'[fiche image=<IMAGE DE VOTRE FICHE> titre=<TITRE DE VOTRE FICHE>]\',\'\',\'[/fiche]\');" title="Cr&eacute;er une fiche" class="d340">Fiche</a>
					</div>
					<br />
					...
					<div class="wiki_p"><strong>Justification de la modification</strong></div>
					<textarea name="commentaire" id="textarea1">'.$erreurjustification.'</textarea>
					<br />
					<input name="submit" type="submit" value="Aper&ccedil;u avant validation" class="ok2" />
					<input name="titre" type="hidden" value="'.$erreurtitre.'" />
