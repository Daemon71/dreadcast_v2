...
$sql = 'SELECT tractes FROM cerclesliste_tbl WHERE nom= "'.$_SESSION['cercle'].'"' ;
$req = mysql_query($sql);
...
if($res>0)
	...
	if($_GET['ok']==1)
		...
		if((ereg("tout",$bddc)) || (ereg("tractes",$bddc)))
			...
			if($capital>=500)
				...
				$capital = $capital - 500;
				$tractes = $tractes + 100;
				$sql = 'UPDATE cerclesliste_tbl SET capital= "'.$capital.'" , tractes= "'.$tractes.'" WHERE nom= "'.$_SESSION['cercle'].'"' ;
				$req = mysql_query($sql);
				}
			else
				{
				print('<strong>Vous n\'avez pas assez d\'argent dans le capital du cercle.</strong><br /><br />');
				}
			}
		else
			{
	...
	print('<strong>Nombre de tractes actuellement</strong>: '.$tractes.'
		<br /><br /><a href="engine=cercletractes.php?ok=1">Acheter 100 tractes</a> (500 Crédits)');
	}
else
	{
	print('<meta http-equiv="refresh" content="0 ; url=http://v2.dreadcast.net/ingame/engine=cercle.php"> ');
