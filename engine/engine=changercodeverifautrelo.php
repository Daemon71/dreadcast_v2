...
if($res!=0)
	...
	if(($nouveau!="") && ($exit!=1))
		...
		if(strlen($nouveau)==strlen($code))
			{
			$sql = 'UPDATE lieu_tbl SET code="'.$nouveau.'" WHERE rue= "'.$_SESSION['autrelor'].'" AND num= "'.$_SESSION['autrelon'].'"' ;
			$req = mysql_query($sql);
			print('<i>Le digicode a correctement &eacute;t&eacute; chang&eacute;.</i>');
			}
		else
			{
