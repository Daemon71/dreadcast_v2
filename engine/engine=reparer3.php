...
$id_lieu = $_SERVER['QUERY_STRING'];
$sql = 'SELECT num, rue FROM lieu_tbl WHERE id= '.$id_lieu;
$req = mysql_query($sql);

if (!mysql_num_rows($req) || (mysql_num_rows($req) && mysql_result($req, 0, num) != $num && strtolower(mysql_result($req, 0, num)) != strtolower($lieu))) {
	mysql_close($db);
	print('<meta http-equiv="refresh" content="0 ; url=http://v2.dreadcast.net/ingame/engine.php"> ');
	exit();
}

if (!(estCasse($id_lieu, 2) && estDroide() && event(4))) {
	mysql_close($db);
	print('<meta http-equiv="refresh" content="0 ; url=http://v2.dreadcast.net/ingame/engine.php"> ');
	exit();
}
...
Vous avez d&eacute;sinfect&eacute; le digicode avec succ&egrave;s !
