...
?>
			...
			<div id="panneau_chat"<?php if($panneau_chat == "off") print(' style="display:none;"'); ?>>
				...
				<?php
					...
					$sql = 'SELECT posteur,message FROM chat WHERE num="'.mysql_result($req,0,num).'" AND rue="'.mysql_result($req,0,rue).'" ORDER BY ids DESC' ;
					$req = mysql_query($sql);
					$res = mysql_num_rows($req);
					
					$res = ($res<11)?$res:11;
					
					for($i=$res-1;$i>=0;$i--) print('<strong>'.mysql_result($req,$i,posteur).':</strong> '.mysql_result($req,$i,message).'<br />');
					
					$sql = 'SELECT id FROM principal_tbl WHERE connec="oui" AND num="'.$_SESSION['num'].'" AND rue="'.$_SESSION['rue'].'"';
					$req = mysql_query($sql);
				...
				<div id="nb_co"><?php print($nb_co); ?></div><a href="engine=chat.php" class="lien"></a>
			</div>
