...
if($exit==1 || $exit==2)
	{
	print('<br /><strong>Vous ne pouvez poser une crosse que sur une arme de tir équipée.</strong>');
	}
elseif($modif1==2 || $modif2==2 || $modif3==2)
	{
...
for($i=1; $i != 7; $i++)
	...
	if(($_SESSION['case'.$i.'']=="Crosse") && ($l!=1))
		...
		if($modif1==0)
			...
			$db = mysql_connect('localhost', 'CENSURE', 'CENSURE') or die("Erreur de connexion au serveur des messages.");
			mysql_select_db('CENSURE',$db) or die("Erreur de connexion a la base de donnees des messages.");
			$sqla = 'UPDATE armes_tbl SET modif1="2" WHERE idarme= "'.strstr($_SESSION['arme'],"-").'"';
			$reqa = mysql_query($sqla);
			$sqla = 'UPDATE principal_tbl SET case'.$i.'="Vide" WHERE id= "'.$_SESSION['id'].'"';
			$reqa = mysql_query($sqla);
			mysql_close($db);
			print("<center><strong>Vous venez d'installer une crosse sur votre arme.</strong><br />Equipée, cette arme confère un bonus de 5 points en tir.</center><br>");
			}
		elseif($modif2==0)
			{
			$l = 1;
			$db = mysql_connect('localhost', 'CENSURE', 'CENSURE') or die("Erreur de connexion au serveur des messages.");
			mysql_select_db('CENSURE',$db) or die("Erreur de connexion a la base de donnees des messages.");
			$sqla = 'UPDATE armes_tbl SET modif2="2" WHERE idarme= "'.strstr($_SESSION['arme'],"-").'"';
			$reqa = mysql_query($sqla);
			$sqla = 'UPDATE principal_tbl SET case'.$i.'="Vide" WHERE id= "'.$_SESSION['id'].'"';
			$reqa = mysql_query($sqla);
			mysql_close($db);
		...
		elseif($modif3==0)
			...
			$db = mysql_connect('localhost', 'CENSURE', 'CENSURE') or die("Erreur de connexion au serveur des messages.");
			mysql_select_db('CENSURE',$db) or die("Erreur de connexion a la base de donnees des messages.");
			$sqla = 'UPDATE armes_tbl SET modif3="2" WHERE idarme= "'.strstr($_SESSION['arme'],"-").'"';
			$reqa = mysql_query($sqla);
			$sqla = 'UPDATE principal_tbl SET case'.$i.'="Vide" WHERE id= "'.$_SESSION['id'].'"';
			$reqa = mysql_query($sqla);
			mysql_close($db);
	...
	{
	print('<br />Voulez-vous poser une crosse sur votre arme équipée ?<br />(+5 tir)<br /><br /><strong>Arme:</strong> <i>'.$_SESSION['arme'].'</i><br /><br /><a href="?ok=1">Oui</a> - <a href="engine=inventaire.php">Non</a>');
	}
