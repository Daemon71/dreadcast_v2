...
$num = ($_SESSION['num'] < 0)?'-1':$_SESSION['num'];
$_SESSION['numtmp'] = $num;

$sql = 'SELECT chat,code FROM lieu_tbl WHERE rue= "'.$_SESSION['lieu'].'" AND num= "'.$num.'"' ;
$req = mysql_query($sql);
...
function afficher()
{
        ...
        $chat = "";
        $sql = 'SELECT posteur,message,moment,ids FROM chat WHERE rue= "'.$_SESSION['lieu'].'" AND num="'.$_SESSION['numtmp'].'" ORDER BY ids ASC';
...
if($message!="")
	...
	$db = mysql_connect('localhost', 'CENSURE', 'CENSURE') or die("Erreur de connexion au serveur des messages.");
	mysql_select_db('CENSURE',$db) or die("Erreur de connexion a la base de donnees des messages.");
	$sql = 'SELECT posteur,message,moment FROM chat WHERE rue="'.$_SESSION['lieu'].'" AND num="'.$num.'" ORDER BY ids';
	$req = mysql_query($sql);
	...
	for($i=1 ; $i!=15 ; $i++)
		{
		$sql = 'UPDATE chat SET moment= "'.mysql_result($req,$i,moment).'" , posteur= "'.mysql_result($req,$i,posteur).'" , message= "'.mysql_result($req,$i,message).'" WHERE ids= "'.$i.'" AND num= "'.$num.'" AND rue= "'.$_SESSION['lieu'].'"';
		mysql_query($sql);
		}
	$sql = 'UPDATE chat SET moment= "'.date("H:i").'" , posteur= "'.$_SESSION['pseudo'].'" , message= "'.$message.'" WHERE ids= "15" AND num= "'.$num.'" AND rue= "'.$_SESSION['lieu'].'"';
	mysql_query($sql);
...
$sql1 = 'SELECT pseudo FROM principal_tbl WHERE connec="oui" AND action!="mort" AND rue="'.$_SESSION['rue'].'" AND num="'.$_SESSION['num'].'"';
$req1 = mysql_query($sql1);
