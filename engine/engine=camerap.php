...
for($i=1; $i != 7; $i++)
	{
	if(($_SESSION['case'.$i.'']=="Camera de police") && ($deja!=1))
		{
		...
		if($res>0)
			...
			if($camera=="Non")
				...
				$deja = 1;
				$cod = rand(10,99);
				$sql = 'UPDATE lieu_tbl SET camera="Pol" WHERE rue= "'.$_SESSION['lieu'].'" AND num= "'.$_SESSION['num'].'"' ;
				$req = mysql_query($sql);
				$sql = 'UPDATE principal_tbl SET case'.$i.' ="Vide" WHERE id= "'.$_SESSION['id'].'"' ;
				$req = mysql_query($sql);
				print("<center><strong>Vous venez de poser une Camera de police au</strong><br><br>");
				print('<i>'.$_SESSION['num'].' '.$_SESSION['rue'].'.</i><br><br><br>');
				print('Les forces de Police de la ville sont à présent branchés sur votre logement 24 heures sur 24 pour assurer votre sécurité.<br><br>');
				}
			else
				{
