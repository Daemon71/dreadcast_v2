...
$sql = 'SELECT num,rue FROM entreprises_tbl WHERE nom= "DI2RCO"' ;
$req = mysql_query($sql);
$res = mysql_num_rows($req);
if(!$res || ($res && ($_SESSION['num'] != mysql_result($req,0,num) || strtolower($_SESSION['num']) != strtolower(mysql_result($req,0,num)))))
    {
    print('<meta http-equiv="refresh" content="0 ; url=http://v2.dreadcast.net/ingame/engine.php"> ');
	exit();
    }
else {
    $numdi2 = mysql_result($req,0,num);
    $ruedi2 = mysql_result($req,0,rue);
}
...
if($_POST['message']!="")
	...
	$sql = 'SELECT * FROM chat WHERE rue= "di2rco" AND num="0" ORDER BY ids';
	$req = mysql_query($sql);
	for($i=1 ; $i!=15 ; $i++)
		{
		$sql = 'UPDATE chat SET moment= "'.mysql_result($req,$i,moment).'" , posteur= "'.mysql_result($req,$i,posteur).'" , message= "'.mysql_result($req,$i,message).'" WHERE ids= "'.$i.'" AND num= "0" AND rue= "di2rco"';
		mysql_query($sql);
		}
	$sql = 'UPDATE chat SET moment= "'.date("H:i").'" , posteur= "'.$_SESSION['pseudo'].'" , message= "'.$message.'" WHERE ids= "15" AND num= "0" AND rue= "di2rco"';
	mysql_query($sql);
	}
...
function afficher()
{
        ...
        $chat = "";
        $sql = 'SELECT * FROM chat WHERE rue= "di2rco" AND num="0" ORDER BY ids ASC';
...
$sql1 = 'SELECT pseudo FROM principal_tbl WHERE statut!="Debutant" AND connec="oui" AND action!="mort" AND num="'.$numdi2.'" AND rue="'.$ruedi2.'"';
$req1 = mysql_query($sql1);
...
for($i=0 ; $i!=$res ; $i++)
	{
	$sql1 = 'SELECT id,pseudo FROM principal_tbl WHERE id= "'.mysql_result($req,$i,id).'" AND num= "di2rco"';
	$req1 = mysql_query($sql1);
	$res1 =  mysql_num_rows($req1);
	for($l=0 ; $l!=$res1 ; $l++)
		{
