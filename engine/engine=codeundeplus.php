...
$req = mysql_query($sql);
$_SESSION['code'] = mysql_result($req,0,code); 

if(strlen($_SESSION['code'])<6)
	{
	if($_SESSION['credits']>179)
		...
		$req = mysql_query($sql);
		$sql = 'UPDATE lieu_tbl SET code="'.$_SESSION['code'].'0" WHERE rue= "'.$_SESSION['ruea'].'" AND num= "'.$_SESSION['numa'].'"' ;
		$req = mysql_query($sql);
		$nbrec = strlen($_SESSION['code']) + 1;
		print('<strong>Votre digicode contient maintenant '.$nbrec.' chiffres. Il devient : '.$_SESSION['code'].'0</strong>');
		mysql_close($db);
		}
	else
		{
