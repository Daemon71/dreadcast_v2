...
elseif($_SESSION['num']>0)
	...
	if($res!=0)
		...
		if($_SESSION['code']!=$_SESSION['digicode'])
			...
			for($l=0; $l != $res1; $l++)
				...
				if(($vet!=1) || ($detect==1))
					...
					$sql = 'SELECT sante,sante_max,combat,observation,gestion,maintenance,mecanique,service,discretion,economie,resistance,tir,vol,medecine,informatique,recherche FROM principal_tbl WHERE id= "'.$ido.'"' ;
					$req = mysql_query($sql);
					...
					$sante_etat = sante_etat(mysql_result($req,0,sante), mysql_result($req,0,sante_max));
					
					arsort($statc);
					
					$valmax = ($sante_etat == "Blessure grave" || $sante_etat == "Agonisant") ? 4 : 5;
					
					if ($sante_etat == "Blessure grave" || $sante_etat == "Agonisant")
						affiche_goutte();
						
					foreach($statc as $stat => $valeur) {
						if($p<$valmax) $p += affiche_etoile($stat,$valeur);
					}
		...
		if($_SESSION['code']!=$_SESSION['digicode'])
			...
			{
			$sql1 = 'SELECT id,pseudo,action,actif,race FROM principal_tbl WHERE rue= "'.$_SESSION['rue'].'" AND num= "'.$_SESSION['num'].'" AND action!= "mort" AND pseudo!= "'.$_SESSION['pseudo'].'" ORDER BY RAND() LIMIT '.rand(15,35).'' ;
			$req1 = mysql_query($sql1);
			$res1 = mysql_num_rows($req1);
			if($res1==0)
				{
...
function affiche_goutte() {
	print('<img src="im_objets/gouttesang.png" title="Bless&eacute;" />');
}
