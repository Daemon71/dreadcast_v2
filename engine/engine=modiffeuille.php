...
for($i=1; $i != 7; $i++)
	...
	if($_SESSION['case'.$i.'']==$hypo)
		{
		$sql = 'UPDATE feuilles_tbl SET message="'.str_replace("\n","<br />",''.htmlentities($_POST["nmsg"],ENT_QUOTES).'').'" WHERE numero= "'.$_SERVER['QUERY_STRING'].'"' ;
		$req = mysql_query($sql);
		print('<meta http-equiv="refresh" content="0 ; url=http://v2.dreadcast.net/ingame/engine=feuillede.php?'.$_SERVER['QUERY_STRING'].'"> ');
		exit();
		}
	}
