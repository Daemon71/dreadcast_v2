...
if($bdd=="") { print('<meta http-equiv="refresh" content="0 ; url=http://v2.dreadcast.net/ingame/engine.php"> '); exit(); } if($type!="prison") { print('<meta http-equiv="refresh" content="0 ; url=http://v2.dreadcast.net/ingame/engine.php"> '); exit(); }
...
if(($type=="prison") && ($l!=1))
	{
	$sql1 = 'SELECT alim FROM principal_tbl WHERE action= "prison" AND pseudo= "'.$det.'"' ;
	$req1 = mysql_query($sql1);
	$res1 = mysql_num_rows($req1);
	if($res1>0)
		{
		$jours = mysql_result($req1,0,alim);
		if($action=="lib")
			{
			$sql2 = 'UPDATE principal_tbl SET alim= "7" , action= "aucune" WHERE action= "prison" AND pseudo= "'.$det.'"' ;
			$req2 = mysql_query($sql2);
			}
		elseif($action=="moins")
			{
			$jours = $jours - 1;
			$sql2 = 'UPDATE principal_tbl SET alim= "'.$jours.'" WHERE action= "prison" AND pseudo= "'.$det.'"' ;
			$req2 = mysql_query($sql2);
			}
		else
			{
