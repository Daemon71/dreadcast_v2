...
if($_SESSION['entreprise']=="DI2RCO" && $_SESSION['type']=="Directeur du DI2RCO")
	...
	if($l!=1)
		{
		print('Cette personne ne possède pas de Laissez-passer.');
		}
	else
		{
		$sql = 'INSERT INTO messages_tbl(id,auteur,cible,message,objet,moment,nouveau) VALUES("","Dreadcast","'.$cible.'","Un agent du DI2RCO vient de désactiver votre Laissez-passer Impérial.","Désactivation LPI","'.time().'","oui")' ;
		$req = mysql_query($sql);
		print('Le Laissez-passer a été correctement désactivé.');
		}
	}
else
	{
	print('Seul le Directeur du DI2RCO peut ordonner la désactivation d\'un Laissez-passer.');
	}
