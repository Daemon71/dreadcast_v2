...
if($_SESSION['medecine']>=100)
	{
	$okmedecine = 1;
	}

if(($okmedecine==1) && ($okmed==1) && ($okInj==1) && ($okKro==1))
	{
	$sql = 'UPDATE principal_tbl SET case'.$o.'= "Vaccin" WHERE id= "'.$_SESSION['id'].'"' ;
	$req = mysql_query($sql);
	}
...
if(($okmedecine==1) && ($okmed==1) && ($okInj==1) && ($okKro==1))
	{
	print('Vous avez réussi à fabriquer une dose de Vaccin.<br /><br /><img src="im_objets/vaccin.jpg" border="0"><br /><br />L\'utiliser permet de vous guerir du virus mortel "Le Faucheur".');
	}
else
	{
	print('Vous n\'avez pas les composants necessaires pour fabriquer du Vaccin.');
	}
?>
