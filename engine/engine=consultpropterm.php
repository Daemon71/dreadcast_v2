...
if(($type=="chambre") && ($l!=1))
	{
	$sql = 'SELECT intitule FROM loisprop_tbl WHERE id= "'.$_GET['id'].'"' ;
	$req = mysql_query($sql);
	$res = mysql_num_rows($req);
	if($res>0)
		{
		$vintitule = mysql_result($req,0,intitule);
		$sql1 = 'SELECT id FROM loisvotes_tbl WHERE intitule= "'.$ventreprise.'" AND membre= "'.$_SESSION['pseudo'].'"' ;
		$req1 = mysql_query($sql1);
		$res1 = mysql_num_rows($req1);
		if($res1==0)
			{
			$sql2 = 'INSERT INTO loisvotes_tbl(id,membre,intitule,vote) VALUES("","'.$_SESSION['pseudo'].'","'.$vintitule.'","'.$_GET['vote'].'") ' ;
			$req2 = mysql_query($sql2);
			print('<p align="center">Vous venez de voter une Loi.<br>Vous pouvez à tout instant regarder où en sont les votes sur votre ordinateur.</p>');
			}
		else
			{
