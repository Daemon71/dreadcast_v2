...
<div id="haut">
	...
	<div class="haut_aligngauche">
		...
		<b class="module4ie"><a href="engine=cerclepostes.php?id=<?php print(''.$_GET['id'].''); ?>" class="module4"><img src="im_objets/icon_retour.gif" alt="Retour" />Retour</a></b>
</p>
	</div>
</div>
<div id="centre">
...
$sql = 'SELECT description FROM `c_'.str_replace(" ","_",''.$_SESSION['cercle'].'').'_tbl` WHERE id= "'.$_GET['id'].'"' ;
$req = mysql_query($sql);
...
if($res>0)
	...
	if($_GET['modif']=="oui")
		...
		if((ereg("tout",$bddc)) || (ereg("postes",$bddc)))
			{
			$sql = 'UPDATE `c_'.str_replace(" ","_",''.$_SESSION['cercle'].'').'_tbl` SET description= "'.str_replace("\n","<br />",''.htmlentities($_POST["description"],ENT_QUOTES).'').'" WHERE id= "'.$_GET['id'].'"' ;
			$req = mysql_query($sql);
			print('<strong>La description est modifiée.</strong><br />(<a href="engine=cercle.php">Retour à la page principale</a>)');
			}
		else
			{
			print('<meta http-equiv="refresh" content="0 ; url=http://v2.dreadcast.net/ingame/engine=cercle.php"> ');
		...
		{
		print('<form name="form1" method="post" action="engine=cerclemdp.php?id='.$_GET['id'].'&modif=oui">
			<strong>Description:</strong>
			<br>
			<textarea name="description" cols="40" rows="9" id="description">'.str_replace("<br />","",mysql_result($req,0,description)).'</textarea>
			<br>
