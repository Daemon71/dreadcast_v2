...
$sql = 'SELECT id,rue,num,action,alim,drogue,sante,sante_max,fatigue,soins FROM principal_tbl WHERE pseudo= "'.$_SESSION['pseudo'].'"' ;
$req = mysql_query($sql);
...
for($i=1; $i != 7; $i++)
	{
	if(($_SESSION['case'.$i.'']=="Medpack") && ($l!=1))
		{
		if($_SESSION['soins']>=40)
			{
			$l = 1;
			print("<center><strong>Vous avez déjà trop de bandages.</strong></center><br>");
			}
		elseif($_SESSION['sante']<$sante_max)
			{
			$l = 1;
			$cod = rand(0,20);
			$_SESSION['sante'] = $_SESSION['sante'] + 30 + $cod;
			if($_SESSION['sante']>$sante_max)
				{
				$_SESSION['sante'] = $sante_max;
				}
			$_SESSION['soinsf'] = $_SESSION['soins'] + 30 + $cod;
			$sql = 'UPDATE principal_tbl SET soins= "'.$_SESSION['soinsf'].'" , sante="'.$_SESSION['sante'].'" WHERE pseudo= "'.$_SESSION['pseudo'].'" AND num= "'.$_SESSION['num'].'"' ;
			$req = mysql_query($sql);
			$sql = 'UPDATE principal_tbl SET case'.$i.' ="Vide" WHERE id= "'.$_SESSION['id'].'"' ;
			$req = mysql_query($sql);
			print('<p align="center"><strong>Vous venez d\'utiliser un medpack.</strong><br><br>');
			print('Votre nouvel &eacute;tat de sant&eacute; est <i>'.$_SESSION['sante'].'/'.$sante_max.'</i>.<br><br>');
			}
		else
			{
			$l = 1;
			print("<center><strong>Vous &ecirc;tes en bonne sant&eacute;.</strong></center><br>");
			}
		}
	}
