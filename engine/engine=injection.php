...
for($i=1; $i != 7; $i++)
	...
	if(($_SESSION['case'.$i.'']=="Injection") && ($l!=1))
		...
		if($_SESSION['discretion']<$discretion_max)
			...
			print('Votre compétence en discretion est maintenant de '.floor($_SESSION['discretion']).'/'.$discretion_max.'.</p>');
			}
		else
			{
			$l = 1;
			print('<center><strong>Vous êtes déjà à '.$discretion_max.'/'.$discretion_max.' en discretion !</strong></center><br>');
			}
		}
	}
