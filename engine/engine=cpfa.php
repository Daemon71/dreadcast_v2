...
if($l != 1)
	...
	$bdd = mysql_result($req1,0,bdd);
	
	if($_SESSION['droit']!="")
		{
		$sql = 'INSERT INTO `e_'.str_replace(" ","_",''.$_SESSION['entreprise'].'').'_tbl` (id,poste,type,salaire,bdd,nbrepostes,nbreactuel,competence,mincomp,candidature,mintrav,sinon,bonus) VALUES("","'.$_SESSION['poste'].'","'.$_SESSION['tposte'].'","'.$_SESSION['salaire'].'","'.$bdd.'","'.$_SESSION['nbrep'].'","","'.$_SESSION['tposte'].'","'.$_SESSION['mincomp'].'","'.$_SESSION['cand'].'","'.$_SESSION['mintrav'].'","'.$_SESSION['sinon'].'","'.$_SESSION['hs'].'")';
		mysql_query($sql);
		}
	else
		{
