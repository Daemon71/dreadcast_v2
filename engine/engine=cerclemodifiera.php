...
$sql = 'SELECT titre,contenu FROM cerclesnews_tbl WHERE id= "'.$_GET['id'].'" AND cercle= "'.$_SESSION['cercle'].'"' ;
$req = mysql_query($sql);
...
if($res>0)
	...
	if($_GET['modif']=="oui")
		...
		if((ereg("tout",$bddc)) || (ereg("actualite",$bddc)))
			{
			$sql = 'UPDATE cerclesnews_tbl SET titre= "'.$_POST['titre'].'" , contenu= "'.str_replace("\n","<br />",''.htmlentities($_POST["contenu"],ENT_QUOTES).'').'" WHERE id= "'.$_GET['id'].'"' ;
			$req = mysql_query($sql);
			print('<strong>Votre nouvelle est modifiée.</strong><br />(<a href="engine=cercleactu.php">Retour à l\'actualité</a>)');
			}
		else
			{
			print('<meta http-equiv="refresh" content="0 ; url=http://v2.dreadcast.net/ingame/engine=cercle.php"> ');
		...
		{
		print('<form name="form1" method="post" action="engine=cerclemodifiera.php?id='.$_GET['id'].'&modif=oui">
			Titre: 
			<input name="titre" type="text" id="titre" size="20" maxlength="100" value= "'.mysql_result($req,0,titre).'">
			<br><br>
			Contenu:
			<br>
			<textarea name="contenu" cols="40" rows="7" id="contenu">'.str_replace("<br />","",mysql_result($req,0,contenu)).'</textarea>
			<br>
