...
$sql = 'SELECT description FROM cerclesliste_tbl WHERE nom= "'.$_SESSION['cercle'].'"' ;
$req = mysql_query($sql);
...
if($res>0)
	...
	if($_GET['modif']=="oui")
		...
		if((ereg("tout",$bddc)) || (ereg("description",$bddc)))
			{
			$sql = 'UPDATE cerclesliste_tbl SET description= "'.str_replace("\n","<br />",''.htmlentities($_POST["description"],ENT_QUOTES).'').'" WHERE nom= "'.$_SESSION['cercle'].'"' ;
			$req = mysql_query($sql);
		...
		{
		print('<form name="form1" method="post" action="engine=cercledescription.php?modif=oui">
