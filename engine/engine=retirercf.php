...
$sql = 'SELECT type,nom,budget FROM entreprises_tbl WHERE rue= "'.$_SESSION['rue'].'" AND num= "'.$_SESSION['num'].'"' ;
$req = mysql_query($sql);
...
if(($ret>$solde) || ($ret<=0))
	{
	print('<meta http-equiv="refresh" content="0 ; url=http://v2.dreadcast.net/ingame/engine=retirerc.php?'.$code.'"> ');
	exit();
	}

$solde = $solde - $ret;
$_SESSION['credits'] = $_SESSION['credits'] + $ret;
...
$sql = 'INSERT INTO comptes_acces_tbl(id,pseudo,compte,time,operation,valeur) VALUES("","'.$_SESSION['pseudo'].'","'.$code.'","'.time().'","Retrait argent","'.$ret.'")' ;
mysql_query($sql);
...
<div id="centre_banque">
	...
	<p id="textelse">Vous avez retir&eacute; <b><?php print(''.$ret.''); ?></b> Cr&eacute;dits.<br />
