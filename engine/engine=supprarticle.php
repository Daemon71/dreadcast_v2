...
if($_SESSION['statut']=="Administrateur" OR $l!=1)
	...
	if($res>0)
		...
		if($_POST['raison']!="")
			{
			$sql = 'INSERT INTO messages_tbl(id,auteur,cible,message,objet,moment) VALUES("","Dreadcast","'.mysql_result($req,0,auteur).'","Votre article intitulé '.mysql_result($req,0,titre).' à été refusé par le DreadCast Network.<br /><br /><strong>Motif:</strong> '.htmlentities(stripslashes($_POST['raison'])).'","Article refusé","'.time().'")' ;
			$req = mysql_query($sql);
