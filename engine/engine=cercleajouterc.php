...
<div id="haut">
	...
	<div class="haut_aligngauche">
		...
		<b class="module4ie"><a href="engine=cerclecom.php?id=<?php print(''.$_GET['id'].''); ?>" class="module4"><img src="im_objets/icon_retour.gif" alt="Retour" />Retour</a></b>
</p>
	</div>
</div>
<div id="centre">
...
if($_GET['id']!="")
	...
	if($_GET['ajout']=="oui")
		...
		if((ereg("tout",$bddc)) || (ereg("commentaires",$bddc)))
			{
			$sql = 'INSERT INTO cerclescom_tbl(id,commentaire,auteur,moment,nouvelle) VALUES("","'.str_replace("\n","<br />",''.htmlentities($_POST["contenu"],ENT_QUOTES).'').'","'.$_SESSION['pseudo'].'","'.time().'","'.$_GET['id'].'")' ;
			$req = mysql_query($sql);
			print('<strong>Votre commentaire est posté.</strong><br />(<a href="engine=cerclecom.php?id='.$_GET['id'].'">Retour à l\'actualité</a>)');
			enregistre($_SESSION['pseudo'],'acc_comms_cercle','+1');
			}
		else
			{
			print('<meta http-equiv="refresh" content="0 ; url=http://v2.dreadcast.net/ingame/engine=cercle.php"> ');
		...
		{
		print('<form name="form1" method="post" action="engine=cercleajouterc.php?id='.$_GET['id'].'&ajout=oui">
			Commentaire:
			<br>
