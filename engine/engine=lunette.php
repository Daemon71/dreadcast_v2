...
if($exit==1 || $exit==2)
	{
	print('<br /><strong>Vous ne pouvez poser une lunette que sur une arme de tir équipée.</strong>');
	}
elseif($modif1==3 || $modif2==3 || $modif3==3)
	{
...
for($i=1; $i != 7; $i++)
	{
	if(($_SESSION['case'.$i.'']=="Lunette") && ($l!=1))
		{
		if($modif1==0)
			{
			$l = 1;
			$db = mysql_connect('localhost', 'CENSURE', 'CENSURE') or die("Erreur de connexion au serveur des messages.");
			mysql_select_db('CENSURE',$db) or die("Erreur de connexion a la base de donnees des messages.");
			$sqla = 'UPDATE armes_tbl SET modif1="3" WHERE idarme= "'.strstr($_SESSION['arme'],"-").'"';
			$reqa = mysql_query($sqla);
			$sqla = 'UPDATE principal_tbl SET case'.$i.'="Vide" WHERE id= "'.$_SESSION['id'].'"';
			$reqa = mysql_query($sqla);
			mysql_close($db);
			print("<center><strong>Vous venez d'installer une lunette sur votre arme.</strong><br />Cette arme possède maintenant une portée supérieure de 10m.</center><br>");
			}
		elseif($modif2==0)
			{
			$l = 1;
			$db = mysql_connect('localhost', 'CENSURE', 'CENSURE') or die("Erreur de connexion au serveur des messages.");
			mysql_select_db('CENSURE',$db) or die("Erreur de connexion a la base de donnees des messages.");
			$sqla = 'UPDATE armes_tbl SET modif2="3" WHERE idarme= "'.strstr($_SESSION['arme'],"-").'"';
			$reqa = mysql_query($sqla);
			$sqla = 'UPDATE principal_tbl SET case'.$i.'="Vide" WHERE id= "'.$_SESSION['id'].'"';
			$reqa = mysql_query($sqla);
			mysql_close($db);
		...
		elseif($modif3==0)
			...
			$db = mysql_connect('localhost', 'CENSURE', 'CENSURE') or die("Erreur de connexion au serveur des messages.");
			mysql_select_db('CENSURE',$db) or die("Erreur de connexion a la base de donnees des messages.");
			$sqla = 'UPDATE armes_tbl SET modif3="3" WHERE idarme= "'.strstr($_SESSION['arme'],"-").'"';
			$reqa = mysql_query($sqla);
			$sqla = 'UPDATE principal_tbl SET case'.$i.'="Vide" WHERE id= "'.$_SESSION['id'].'"';
			$reqa = mysql_query($sqla);
			mysql_close($db);
	...
	{
	print('<br />Voulez-vous poser une lunette sur votre arme équipée ?<br />(portée +10m)<br /><br /><strong>Arme:</strong> <i>'.$_SESSION['arme'].'</i><br /><br /><a href="?ok=1">Oui</a> - <a href="engine=inventaire.php">Non</a>');
	}
