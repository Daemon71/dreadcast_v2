...
if($res>0)
	...
	if((ereg("tout",$bddc)) || (ereg("digicode",$bddc)))
		...
		elseif(mysql_result($reqc,0,capital)>=180)
			...
			$reqc = mysql_query($sqlc);
			print('<strong>Un nouveau chiffre est maintenant disponible.</strong><br />(<a href="engine=cerclestatut.php">Retour à la page des statuts</a>)');
			}
		else
			{
			print('<strong>Il faut plus de Crédits dans le capital du cercle pour pouvoir acheter une nouveau chiffre.</strong>');
			}
		}
	else
		{
